...
2017-01-19:
    ...
    TODO:
        ...
        增加快速按日期来查找命令
            kdy year/y   列出当前年所有记录或指定年所有记录
            kdy month/m  列出当前月所有记录或指定月记录
            kdy day/d    列出当前天所有记录或指定日期记录
            kdy week/w   列出本周所有记录
            kdy time     别名kdy list -t功能
            默认按修改时间来查找，可以通过环境变量kdytime来指定按什么时候来查找
        增加kdy支持限制记录数列表
        增加列表选择编辑函数
        限制查找或编辑列表数超过11条就只列表，否则交互选择编辑
        增加交互选择编辑退出功能
